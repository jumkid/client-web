...
export interface IVehicleService {
  getById: (id:string) => Promise<APIResponse<VehicleProfile>>
  ...
  update: (id:string, vehicleProfile:VehicleProfile) => Promise<APIResponse<VehicleProfile>>
...
class VehicleService implements IVehicleService {
  ...
  async getById(id: string): Promise<APIResponse<VehicleProfile>> {
    const url = `${C.VEHICLES_API}/${id}`;
    const response = await restfulClient.getWithPromise(url);
